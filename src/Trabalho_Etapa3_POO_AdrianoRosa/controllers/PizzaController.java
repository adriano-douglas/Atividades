	...
	ActionListener salvarPizza = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (!erroNome && !erroDValidade && !erroPeso && !erroPrecoVenda && !erroRecheios) {
				...
				Pizza pizza = new Pizza(0, nome, precoVendaD, LocalDate.parse(dataValidade, formato), pesoD, molho,
						recheiosUsados, borda);

				int idPizza = PizzaDao.cadastrarPizza(pizza, false);
				if (idPizza == 0) {
					cadastroPizzaPanel.exibirMensagem("Erro ao cadastrar a pizza no banco!");
					return;
				}
				pizza.setId(idPizza);
				DB.pizzas.add(pizza);
